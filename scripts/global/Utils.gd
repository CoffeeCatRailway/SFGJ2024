...
func randomWeighted(elements: Array, weights: Array) -> Variant:
	var _seed: float = randf()
	var cumulativeWeight: float = 0.
	var totalWeight: float = weights.reduce(func(a: float, b: float) -> float: return a + b, 0.)
	for i: float in weights.size():
		cumulativeWeight += weights[i] / totalWeight
		if _seed <= cumulativeWeight:
			return elements[i]
	return null
